...
from ..builder import DINO_MODELS, build_assigner, scope_dino_types
...
class Mask2FormerHead(BaseDecodeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pixel_decoder = scope_dino_types(ConfigDict(pixel_decoder))
        transformer_decoder = scope_dino_types(ConfigDict(transformer_decoder))
        positional_encoding = scope_dino_types(ConfigDict(positional_encoding))
        loss_cls = scope_dino_types(ConfigDict(loss_cls))
        loss_mask = scope_dino_types(ConfigDict(loss_mask))
        loss_dice = scope_dino_types(ConfigDict(loss_dice))
        train_cfg = ConfigDict(train_cfg) if train_cfg is not None else None
        test_cfg = ConfigDict(test_cfg) if test_cfg is not None else None
        ...
        self.pixel_decoder = DINO_MODELS.build(pixel_decoder_)
        self.transformer_decoder = DINO_MODELS.build(transformer_decoder)
        ...
        self.decoder_positional_encoding = DINO_MODELS.build(positional_encoding)
        ...
        self.loss_cls = DINO_MODELS.build(loss_cls)
        self.loss_mask = DINO_MODELS.build(loss_mask)
        self.loss_dice = DINO_MODELS.build(loss_dice)
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list, gt_masks_list, img_metas):
        ...
        all_mask_preds = [prediction.float() for prediction in all_mask_preds]
