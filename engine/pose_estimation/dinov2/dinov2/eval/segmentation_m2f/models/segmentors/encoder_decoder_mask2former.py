...
from ..builder import DINO_MODELS, scope_dino_types
from mmseg.models.utils import resize


@DINO_MODELS.register_module()
class EncoderDecoderMask2Former(BaseSegmentor):
    ...
    def __init__(
        ...
        super(EncoderDecoderMask2Former, self).__init__(
            data_preprocessor=data_preprocessor, init_cfg=init_cfg
        )
        train_cfg = ConfigDict(train_cfg) if train_cfg is not None else None
        test_cfg = ConfigDict(test_cfg) if test_cfg is not None else ConfigDict(mode="whole")
        if pretrained is not None:
            assert backbone.get("pretrained") is None, "both backbone and segmentor set pretrained weight"
            backbone.pretrained = pretrained
        self.backbone = DINO_MODELS.build(scope_dino_types(backbone))
        if neck is not None:
            self.neck = DINO_MODELS.build(scope_dino_types(neck))
    ...
    def _init_decode_head(self, decode_head):
        ...
        self.decode_head = DINO_MODELS.build(scope_dino_types(decode_head))
            ...
            if isinstance(auxiliary_head, list):
                ...
                for head_cfg in auxiliary_head:
                    self.auxiliary_head.append(
                        DINO_MODELS.build(scope_dino_types(head_cfg))
                    )
            else:
                self.auxiliary_head = DINO_MODELS.build(scope_dino_types(auxiliary_head))
    ...
    @staticmethod
    def _data_samples_to_metas(data_samples, inputs):
        if data_samples is None:
            height, width = inputs.shape[-2:]
            return [
                dict(
                    ori_shape=(height, width, inputs.shape[1]),
                    img_shape=(height, width, inputs.shape[1]),
                    pad_shape=(height, width, inputs.shape[1]),
                    flip=False,
                )
    ...
    def _forward(self, inputs, data_samples=None):
        """MMEngine tensor-mode forward."""
        img_metas = self._data_samples_to_metas(data_samples, inputs)
        return self.encode_decode(inputs, img_metas)

    def loss(self, inputs, data_samples):
        """MMEngine loss-mode adapter for the legacy Mask2Former targets."""
        if not data_samples:
            raise ValueError("loss mode requires segmentation data samples")
        img_metas = self._data_samples_to_metas(data_samples, inputs)
        try:
            gt_semantic_seg = torch.stack(
                [sample.gt_sem_seg.data for sample in data_samples]
            )
            gt_labels = [sample.gt_labels for sample in data_samples]
            gt_masks = [sample.gt_masks for sample in data_samples]
        except AttributeError as exc:
            raise AttributeError(
                "DINOv2 Mask2Former training samples require gt_sem_seg, "
                "gt_labels and gt_masks attributes"
            ) from exc
    ...
    def predict(self, inputs, data_samples=None):
        """MMEngine predict-mode adapter returning SegDataSample objects."""
        img_metas = self._data_samples_to_metas(data_samples, inputs)
        seg_logits = self.encode_decode(inputs, img_metas)
