...
from ..builder import DINO_MODELS, TRANSFORMER, scope_dino_types
...
@DINO_MODELS.register_module(force=True)
class FFN(BaseModule):
    ...
    def forward(self, x, identity=None):
        ...
        if self.with_cp and x.requires_grad:
            out = cp.checkpoint(self.layers, x, use_reentrant=False)
        else:
            out = self.layers(x)
...
class Transformer(BaseModule):
    ...
    def __init__(self, encoder=None, decoder=None, init_cfg=None):
        ...
        self.encoder = DINO_MODELS.build(scope_dino_types(encoder))
        self.decoder = DINO_MODELS.build(scope_dino_types(decoder))
